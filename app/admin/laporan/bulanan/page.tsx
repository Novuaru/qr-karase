...
export default function LaporanBulanan() {
  ...
  const exportToExcel = () => {
    ...
    saveAs(blob, `laporan_kasir_bulanan_${new Date().toISOString().slice(0, 10)}.xlsx`)
  }

  return (
    <main className="max-w-5xl mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-red-700">📊 Laporan Bulanan</h1>
        {data.length > 0 && (
          <button
      ...
      {loading && <p className="text-gray-500">Memuat data laporan bulanan...</p>}
      {error && <p className="text-red-600">{error}</p>}

      {!loading && !error && data.length === 0 && (
        <p className="text-gray-500 italic">Tidak ada data laporan bulan ini.</p>
      )}
